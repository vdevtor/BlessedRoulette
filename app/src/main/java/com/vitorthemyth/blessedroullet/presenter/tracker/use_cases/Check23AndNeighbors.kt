...
class Check23AndNeighbors {

    operator fun invoke(list: List<RouletteNumber>): RouletteStrategy? {
        if (list.size < 2) return null

        val triggerNumber = list.find { it.number == "32"} ?: return null
        val triggerIndex = list.indexOf(triggerNumber)
        val targetNumber = provideRouletteNumbers().find { it.number == "23" }
        ...
        return RouletteStrategy(
            strategyTitle = "23 & Vizinhos",
            strategyDescription = "Saiu o número 32 e ele é gatilho do número 23 e vizinhos",
            ...
            strategyType = StrategyType.twentyThreeAndNeighbors,
