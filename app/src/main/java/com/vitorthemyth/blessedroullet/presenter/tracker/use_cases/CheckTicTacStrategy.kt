...
class CheckTicTacStrategy {
    ...
    operator fun  invoke(list :List<RouletteNumber>) : RouletteStrategy?{
          ...
          val strategyDescription = "Jogue o inverso dos dois ultimos numero baseando-se nas cores e altos/baixos"

          val playableNumbers = when {
               isRed && isHigh -> list.filter { !it.isHigherNumber && it.color == Color.Black }
               isBlack && isLow -> list.filter { it.isHigherNumber && it.color == Color.Red }
               isBlack && isHigh -> list.filter { !it.isHigherNumber && it.color == Color.Red }
               isRed && isLow -> list.filter { it.isHigherNumber && it.color == Color.Black }
               else -> emptyList()
          }
